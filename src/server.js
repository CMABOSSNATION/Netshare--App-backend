...
// ── Persistence: save/load access codes to disk ───────────────────────
const DATA_FILE = nodePath.join(__dirname, '..', 'data', 'codes.json');

function saveCodes(codesMap) {
  try {
    const dir = nodePath.dirname(DATA_FILE);
    if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
    const obj = {};
    codesMap.forEach((v, k) => { obj[k] = v; });
    fs.writeFileSync(DATA_FILE, JSON.stringify(obj, null, 2));
  } catch (e) {
    console.warn('[persist] Could not save codes:', e.message);
  }
}
...
function loadCodes() {
  try {
    ...
    console.warn('[persist] Could not load codes:', e.message);
    return new Map();
  }
}
